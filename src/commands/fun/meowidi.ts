...
const SAMPLE_FILES = [
  { file: 'Cat_idle2.wav', baseFreq: 489.8 },
...
function parseWav(buf: Buffer): Float32Array {
  ...
  for (let s = 0; s < totalSamples; s++) mono[s] = buf.readInt16LE(dataOffset + s * 2) / 32768;
  return mono;
}
...
function pitchShift(
  ...
  const playbackRate = Math.max(
    0.1,
    Math.min(10, (targetFreq / baseFreq) * (srcRate / SAMPLE_RATE))
  );
...
function encodeWav(floatBuffer: Float32Array) {
  ...
  for (let i = 0; i < floatBuffer.length; i++) {
    wav.writeInt16LE(
      Math.round(Math.max(-1, Math.min(1, floatBuffer[i] ?? 1)) * 32767),
      44 + i * 2
    );
  }
  return wav;
...
function renderNotes(notes: Note[], sample: Sample) {
  ...
  for (const note of notes) {
    ...
    if (!pitchCache.has(note.midi)) {
      pitchCache.set(
        note.midi,
        pitchShift(sample.samples, sample.sampleRate, midiToFreq(note.midi), sample.baseFreq)
      );
    }
    ...
    for (let i = 0; i < len; i++) {
      ...
      outBuffer[startSample + i] =
        (outBuffer[startSample + i] ?? 0) + pitched[i]! * note.velocity * fadeGain;
    }
  }
  return outBuffer;
}
...
function normalizeBuffer(buffer: Float32Array) {
  ...
  if (maxAbs > 0.01) {
    ...
    for (let i = 0; i < buffer.length; i++) buffer[i] = (buffer[i] ?? 0) * scale;
  }
}
...
async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (!name.endsWith('.mid') && !name.endsWith('.midi')) {
    await interaction.reply({
      content: 'please give me a .mid or .midi file! :(',
